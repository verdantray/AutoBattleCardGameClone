Shader "Neon/Object/ObjectIngameShader_worldPosGradient"
{
    Properties
    {

        [Header(OutlineStencil)]
        [Toggle]_useOutlineStencil("use OutlineStencil", float) = 0
        ...
        [Space(20)]
        [Header(WorldPosGradient)]
        [Space(10)]
        [KeywordEnum(none, xy, yz, xz)] _UvDimension("UV Dimension", Float) = 1
        [KeywordEnum(vertex, pixel)] _ComputeLocation("Compute Locate", Float) = 0
        _GradientTex ("Gradient Texture", 2D) = "black" {}
        [HDR]_GradientTint ("Gradient Tint", Color) = (1, 1, 1, 1)
        _GradientXSpeed("Gradient X Speed", float) = 0
        _GradientYSpeed("Gradient Y Speed", float) = 0
        ...
        [Toggle] _UseScaleCalibration("Use Scale Calibration", float) = 0

        // 글로벌 변수를 사용하면 srp batcher를 사용할 수 없는가?
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //option
            #pragma shader_feature_local _ _USESHADOWCOLORTEXTURE_ON
            #pragma shader_feature_local _ _USENORMALTEX_ON
            #pragma shader_feature_local _ _USEMATCAP_ON
            #pragma shader_feature_local _ _USEOUTLINESTENCIL_ON
            #pragma shader_feature_local _UVDIMENSION_NONE _UVDIMENSION_XY _UVDIMENSION_YZ _UVDIMENSION_XZ
            #pragma shader_feature_local _COMPUTELOCATION_VERTEX _COMPUTELOCATION_PIXEL
            #pragma shader_feature_local _ _USEBASETEXWORLDPOSUV_ON
            ...
            float _ApplyMainTextureColor, _OnlyVertexColorViewMode;
            float _UsePerspectiveCorrect;
            
            float _ZOffset, _ShadowZOffset;

            float _MatcapMixAmount, _MatcapAddAmount, _MatcapOpacity;
            float4 _MatcapColor;
            ...
            float4 _GradientTint;
            float _GradientXSpeed;
            float _GradientYSpeed;
            ...
            struct v2f
            {
                ...
                float3 normal : NORMAL; 
                float4 texcoord : TEXCOORD0; // zw is WorldPos UV
            ...
            v2f vert (appdata v)
            {
                ...
                // WorldPos UV - VERTEX
                {
		            ...
		            #ifdef _COMPUTELOCATION_VERTEX

                        float3 normPositionWS = abs(o.normal);
                        float2 tmpUv = float2(0, 0);

                        #ifdef _UVDIMENSION_NONE
                            if(normPositionWS.x < normPositionWS.y)
                            {
                                if(normPositionWS.y < normPositionWS.z)
                                {
                                    tmpUv = positionWS.xy;
                                    // z가 가장 큼
                                    // y중간
                                ...
                                else if (normPositionWS.x < normPositionWS.z) 
                                { 
                                    tmpUv = positionWS.xz;
                                    // y가 가장 큼
                                    // z 중간
                            ...
                            {
                                if(normPositionWS.x < normPositionWS.z)
                                {
                                    tmpUv = positionWS.xy;
                                    // z가 가장 큼
                                    // x 중간
                                ...
                                else if(normPositionWS.y < normPositionWS.z) 
                                { 
                                    tmpUv = positionWS.yz;
                                    // x가 가장 큼
                                    // z 중간
                            ...
                            tmpUv *=  _GradientTex_ST.xy;
                            o.texcoord.zw = tmpUv;
                        

                        #elif _UVDIMENSION_XY
			                o.texcoord.zw = TRANSFORM_TEX(positionWS.xy, _GradientTex);

		                #elif _UVDIMENSION_YZ
			                o.texcoord.zw = TRANSFORM_TEX(positionWS.yz, _GradientTex);

		                #elif _UVDIMENSION_XZ
			                o.texcoord.zw = TRANSFORM_TEX(positionWS.xz, _GradientTex);
                        #endif
		            #endif
                }
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float4 MainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoord.xy) * _Color;
                float4 Mask = SAMPLE_TEXTURE2D(_ILMMaskTex, sampler_ILMMaskTex, i.texcoord.xy);

                
                //source
                #if _USENORMALTEX_ON
                    float3 NormalTS = UnpackNormal( SAMPLE_TEXTURE2D(_NormalTex, sampler_NormalTex, i.texcoord.xy));
                ...
                c.rgb += lerp( rimLight * lerp(limDefault, MainTex.rgb, _RimColor.a) , 0 , _RimMode);
                ...
                //Gradient Map
                float2 worldUv = i.texcoord.zw;
                
                // WorldPos UV - Pixel
                float3 positionWS = i.worldPos;

		        #ifdef _COMPUTELOCATION_PIXEL
                    
                    float3 normPositionWS = abs(i.normal);
                    ...
                    worldUv *=  _GradientTex_ST;

                    #elif _UVDIMENSION_XY
			            worldUv = TRANSFORM_TEX(positionWS.xy, _GradientTex);

		            #elif _UVDIMENSION_YZ
			            worldUv = TRANSFORM_TEX(positionWS.yz, _GradientTex);

		            #elif _UVDIMENSION_XZ
			            worldUv = TRANSFORM_TEX(positionWS.xz, _GradientTex);
		            #endif
                #endif


                float2 panner = float2(_Time.x * _GradientXSpeed, _Time.x * _GradientYSpeed);
                float3 gradientColor = SAMPLE_TEXTURE2D(_GradientTex, sampler_GradientTex, worldUv + panner ).xyz;
                c.rgb += gradientColor * _GradientTint.rgb;
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _ _USESCALECALIBRATION_ON
            ...
            v2f vert (appdata v)
            {
                ...
                #ifdef _USESCALECALIBRATION_ON
                    float3 worldScale = float3(
                    length(float3(unity_ObjectToWorld[0].x, unity_ObjectToWorld[1].x, unity_ObjectToWorld[2].x)), // scale x axis
                    length(float3(unity_ObjectToWorld[0].y, unity_ObjectToWorld[1].y, unity_ObjectToWorld[2].y)), // scale y axis
                    length(float3(unity_ObjectToWorld[0].z, unity_ObjectToWorld[1].z, unity_ObjectToWorld[2].z))  // scale z axis
                    );
                    float3 positionWS = TransformObjectToWorld(v.vertex.xyz  + (v.normal / worldScale * v.color.r * 0.01 * _OutlineWidth * lerp(1, TransformObjectToHClip(v.vertex.xyz).a ,_UsePerspectiveCorrect)) );
                #else
