Shader "Neon/Object/ObjectIngameShader_Dissolve"
{
    Properties
    {
        //[Header(Stencil)]
        ...
        _DissolveEdgeBold("Dissolve Edge Bold", Range(0, 1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                //Dissolve
                float4 dissolveTex = SAMPLE_TEXTURE2D(_DissolveTex, sampler_DissolveTex, i.texcoord * _DissolveTex_ST.xy + _DissolveTex_ST.zw);
                
                float remapRaito = 1 + _DissolveEdgeBold;
                float cutoff = (dissolveTex.r*0.999) 
                                - _DissolveValue + _DissolveEdgeBold
                                - ((_DissolveValue) * _DissolveEdgeBold);
                float edgeRange = ceil(saturate(cutoff)) - ceil(saturate(cutoff - (_DissolveEdgeBold * 0.999)));

                clip(cutoff);

                return lerp(c, _DissolveEdgeColor, edgeRange);    
            }
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float remapRaito = 1 + _DissolveEdgeBold;
                _DissolveValue = saturate(_DissolveValue * 3);
                float cutoff = (dissolveTex.r*0.999) 
                                - _DissolveValue + _DissolveEdgeBold
                                - ((_DissolveValue) * _DissolveEdgeBold);
                float edgeRange = ceil(saturate(cutoff)) - ceil(saturate(cutoff - (_DissolveEdgeBold * 0.999)));
                ...
                return lerp(col, _DissolveEdgeColor, edgeRange);
                //return col;
            }
            ENDHLSL
        }
